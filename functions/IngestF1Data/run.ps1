...
try {
    ...
    if (-not $env:IDENTITY_ENDPOINT) { throw "IDENTITY_ENDPOINT is missing. Managed Identity is not available." }

    $sqlServer = $env:SQL_SERVER_NAME 
    $sqlDatabase = "sqldb-f1-tracker"

    # 1. Get Managed Identity Token via dynamically injected Azure endpoint
    ...
    try {
        ...
        $transaction.Commit()
        Write-Host "Successfully updated Driver Standings, Constructor Standings, Schedule, and Race Results for Season $season."
    }
    catch {
